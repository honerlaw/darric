...
- [decided] promote partition: PROMOTE 8 (in-process rebuild on rmcp 3; own read-only connection; rowid renumbering constraint; setup outside the Tokio runtime; reqwest 0.13 for rmcp's client; user-event clipboard; one test_db helper; empty-page cursor bug) / MERGE 4 (search shape, nullable seq, local-account reach, live verification) / DISCARD (decision log, triage bookkeeping) / TODO 0 (solo gate)
- [decided] TODO disposition: nothing forward-looking survived the deferral bar; no issue filed, no Closes field (no open followup resolved by this diff) (solo gate)
