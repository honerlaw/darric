...
- [escalated to user] phase-2 ship precondition: user directed "merge everything down into main, then ask me to try it out; we can always fix forward, no one else is using this stuff" — the criterion-2 confirmation gate is waived by the user and phase 2 ships now; the in-app check happens on main afterwards (escalation 3 of 3 — the run halts after this phase's ship and cleanup per the counter rule)
...
- 2026-09-06 (later): the user waived the gate — "merge everything down into main, then ask me
  to try it out … we can always fix forward, no one else is using this stuff". Phase 2 ships;
  the in-app check runs on main with both phases in.
